...
resource oci_load_balancer_load_balancer otmpublb {
  ...
  subnet_ids = [
    oci_core_subnet.otm_lbsub.id,
  ]
}

resource oci_load_balancer_listener otmpublb_wpaspoclblsnr01 {
  connection_configuration {
    backend_tcp_proxy_protocol_version = "0"
    idle_timeout_in_seconds            = "60"
  }
  default_backend_set_name = oci_load_balancer_backend_set.otmpublbbs01.name
  hostname_names = [
  ]
  load_balancer_id = oci_load_balancer_load_balancer.otmpublb.id
...
resource oci_load_balancer_backend_set otmpublbbs01 {
  health_checker {
    interval_ms         = "10000"
    port                = "80"
    protocol            = "TCP"
...
resource oci_load_balancer_backend webbckend {
  count = local.vm_count
  backendset_name  = oci_load_balancer_backend_set.otmpublbbs01.name
  backup           = "false"
  drain            = "false"
...
resource oci_load_balancer_backend_set otm_app_bckend {
  health_checker {
    interval_ms         = "10000"
    port                = "80"
    protocol            = "TCP"
...
resource oci_load_balancer_backend otm_app_pub {
